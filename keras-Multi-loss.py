...
auto_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=3, verbose=0, mode='auto', min_delta=0.0001, cooldown=0, min_lr=0)
save_model = ModelCheckpoint('xception{epoch:02d}-{val_ctg_out_1_acc:.2f}.h5', period=2)
if os.path.exists('Multi_xception.h5'):
    model = load_model('Multi_xception.h5')
else:
    ...
    category_predict1 = Dense(5004, activation='softmax', name='ctg_out_1')(
        Dropout(0.5)(feature1)
    )
    category_predict2 = Dense(5004, activation='softmax', name='ctg_out_2')(
